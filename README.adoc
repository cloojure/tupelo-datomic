...
=== The Tupelo-Datomic API Docs http://cloojure.github.io/doc/tupelo-datomic/tupelo-datomic.core.html[are posted on GitHub Pages]
...
to the `?name` and `?loc` symbols, respectively, and then used to generate an output tuple of the shape 
`[?name ?loc]`. These tuples are added to the result set which will be returned to the caller.  Since
the returned value is a normal Clojure set, duplicate elements are not allowed and any non-unique
...
----
Note that, in both of the calls to `find-entity`, the symbol `?loc` is bound to the string "Caribbean",
while the symbols `?eid` and `?name` are left free. This means the query map in the `:where` clause
will match any entity that posseses all three attributes `:db/id`, `:location`, and `:person/name`
(note that every entity has the `:db/id` attribute by definition).  In addition, only entities 
whose `:location` attribute has the value "Caribbean" will be selected. Once an entity is selected,
its values for the attributes `:db/id` and `:location` are bound to the symbols `?eid` and `?name`, 
respectively, and the output tuple `[?eid ?name]` is added to the result set.
...
#TODO:  This is migrating to `find-pull` soon
